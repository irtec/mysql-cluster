...
# login-path is a new feature supported by MySQL 5.6. Use the mysql_config_editor tool to encrypt and save the authentication information for logging in to the MySQL server in the ~/.mylogin.cnf file v.
# Afterwards, the MySQL client tool can connect to MySQL by reading the encrypted file, avoiding repeated input of login information and exposure of sensitive information.
